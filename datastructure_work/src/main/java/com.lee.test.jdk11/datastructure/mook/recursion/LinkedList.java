package com.lee.test.jdk11.datastructure.mook.recursion;
...
public class LinkedList<E> extends AbstractList<E> implements List<E>, Deque<E> {
    ...
    public void removeElements(E delVal) {
        removeElementsRC(first, delVal);
    }

    public void removeElements(Node<E> head, E delVal) {
        removeElementsRC(head == null ? first : head, delVal);
    }

    private void removeElementsRC(Node<E> head, E delVal) {
        E data = head.data;
        if (data.equals(delVal)) {
            head = removeCurrentNode(head);
            if (head == null) {
                return;
            }
            removeElementsRC(head, delVal);
        }
        if (head.next != null) {
            removeElementsRC(head.next, delVal);
        }
    }


    /**
     * @param node 逻辑设定node必须存在，非空
     * @return
     */
    private Node<E> removeCurrentNode(Node<E> node) {
